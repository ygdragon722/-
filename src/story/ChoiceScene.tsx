...
export default function ChoiceScene({ tag, bg, setup, choices, onChoose, continueLabel = '继续 →' }: Props) {
  ...
  return (
    <div className="relative mx-auto min-h-screen w-full max-w-[440px] overflow-hidden bg-stone-950">
      ...
      {/* 一、铺陈：字幕框，一句一句点着读 */}
      {!picked && !setupDone && (
        <SubtitleBox text={setup} startDelay={bg ? 1100 : 450} onDone={() => setSetupDone(true)} />
      )}
        ...
        <div className="absolute inset-x-0 bottom-0 z-20 space-y-7 px-7 pb-14">
          {choices.map((c, i) => (
            <button
              ...
              className={`block w-full animate-fade-in rounded border px-6 py-6 text-center font-serif text-[16px] leading-8 tracking-[0.05em] backdrop-blur-sm transition-all duration-500 active:scale-[0.98] ${
                selecting === c.id
      ...
      {/* 三、反应：选后她/世界的回应也走字幕框 */}
      {picked && !reactDone && (
        <SubtitleBox key={picked.id} text={picked.outcome} startDelay={400} onDone={() => setReactDone(true)} />
      )}
      ...
      {picked && reactDone && (
        <div className="absolute inset-x-0 bottom-0 z-20 animate-fade-in px-7 pb-14">
          <button
            ...
            className="w-full rounded border border-amber-300/60 py-3 text-[14px] text-amber-100 transition hover:bg-amber-300/10 disabled:opacity-60"
          >
            {continueLabel}
          </button>
        </div>
